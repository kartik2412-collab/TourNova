...
import { listCrowdReports } from "@/lib/catalog/crowd";

export const runtime = "nodejs";

/**
 * GET /api/admin/crowd[?limit=]
 * Reviewer queue of crowd reports (REVIEW_VERIFICATIONS). Lists every
 * USER_REPORTED crowd observation record with its linked submission + submitter so a reviewer
...
export async function GET(request: NextRequest) {
  ...
  const reports = await listCrowdReports(db, { limit });
